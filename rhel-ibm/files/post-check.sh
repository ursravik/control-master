...
LOG_FILE=/tmp/patching-data/postpatch-`uname -n`
LOG=$LOG_FILE
...
(
echo " "
echo "Capture Post  reboot for Server `uname -n` ON `date`"
echo "--------------------------------------------------------------- "
echo " "
echo  "#########  UPTIME STATUS ######## "
uptime
...
echo "######### Capture file systems used above 85% ###########"
df -Th | grep -i -v nfs | grep -i -v cifs | awk '0+$6 >= 85 {print}' > $LOG-usage
